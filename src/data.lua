...
function loadData(set, idx, batchsize)
	...
	for i = 1, opt.nStack do
	    table.insert(label,torch.Tensor(batchsize, opt.resZ[i]*labelDim[1], labelDim[2], labelDim[3]))
	end
        for i = 1, batchsize do
            idx_ = idx or torch.random(annot[set]['nsamples'])
            idx_ = (idx_ + i - 2) % annot[set]['nsamples'] + 1
            input[i],labelTemp = generateSample(set, idx_)
	    for j = 1, opt.nStack do
		label[j][i] = labelTemp[j]
	    end
        end
    end
    ...
    if not isTesting then
	    ...
	    for j = 1, opt.nStack do
		label[j][i] = crop(label[j][i], {(out+1)/2,(out+1)/2}, out*s[i]/200, r[i], out)
	    end
        end
        ...
        if torch.uniform() <= .5 then
            input = flip_(input)
	    for i = 1, opt.nStack do
                label[i] = flip_(shuffleLR_(label[i],matchedParts3D[i]))
	    end
        end
    end
