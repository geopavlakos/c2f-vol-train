...
function drawGaussian3D(vol, pt, z, sigma_2d, size_z)

    local resZ = vol:size(1)
    local res2D = vol:size(2)
    local temp = torch.zeros(res2D,res2D)
    drawGaussian(temp, pt, sigma_2d)
    local zun = image.gaussian(size_z)[torch.ceil(size_z/2)]
    local count = 0
    for i = z-torch.floor(size_z/2),z+torch.floor(size_z/2) do
        count = count+1
        if i>0 and i<resZ+1 then
...
function shuffleLR(x,matchedParts)
    local dim
