...
function step(tag)
    ...
    for i,sample in loader[set]:run() do
		...
		flippedOut = applyFn(function (x) return flip_(shuffleLR_(x,matchedParts3D[1])) end, flippedOut)
	    else
	        for j = 1,opt.nStack do
		    flippedOut[j] = applyFn(function (x) return flip_(shuffleLR_(x,matchedParts3D[j])) end, flippedOut[j])
	        end
	    end
